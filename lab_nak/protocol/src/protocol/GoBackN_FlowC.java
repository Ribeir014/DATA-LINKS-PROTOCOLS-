...
public class GoBackN_FlowC extends Base_Protocol implements Callbacks {
    ...
    private int last;
    /**
     * numero de seq do ultimo nak enviado, para nao haver repeticoes
     */
     ...
     * @param seq 
     */
    private void send_nak() {
        last_nak = prev_seq(frame_expected);
        Frame nframe = Frame.new_Nak_Frame(prev_seq(frame_expected), net.get_recvbuffsize());
    ...
    public void from_physical_layer(long time, Frame frame) {
        ...
        if (frame.kind() == Frame.DATA_FRAME) {     // Check the frame kind
            ...
            if (dframe.seq() == frame_expected) {                                                                
                ...
                if (last_nak != prev_seq(frame_expected)) {
                    sending_nak = true;
                    send_nak();
            ...
            if (between(base, nframe.nak(), next_seq(last))) {
                sim.cancel_data_timer(base);
                sim.Log(Arrays.toString(window_buffer)+"\n");
                clean_buffer(nframe.nak());
                sim.Log(Arrays.toString(window_buffer)+"\n");
                base = next_seq(nframe.nak()); 
                count_retran = base;
                if (nframe.rcvbufsize() != 0) {
                    //quando mudo o buffer e o nak que recebi passa a ser o ultimo que estava a espera
                    if (nframe.nak() == last) {
                        sim.Log("recebi ultimo pacote posso mandar o proximo\n");
                        send_next_data_packet();
                    }
